...
class Agent_Params(enum.Enum):
    planner = {
            "model": Local_sLM_Models.gemma3_4b,     
            "temperature": 0.1,
            "repeat_penalty": 1.20,
            "top_p": 0.95,
            "typical_p": 0.98,
    ...
    researcher = {
            "model": Local_sLM_Models.llama32_2b,       
            "temperature": 0.3,
            "repeat_penalty": 1.18,
            "top_p": 0.95,
            "typical_p": 0.98,
    ...
    refiner = {
            "model": Local_sLM_Models.phi4_mini,      
            "temperature": 0.7,
            # "repeat_penalty": 1.05,
            "repeat_penalty": 1.15,
            "top_p": 0.96,
...
planner: ChatOllama = ChatOllama(**(Agent_Params.planner.value))
researcher: ChatOllama = ChatOllama(**Agent_Params.researcher.value)
refiner: ChatOllama = ChatOllama(**Agent_Params.refiner.value)
...
async def compress_history(messages: List[BaseMessage]) -> List[BaseMessage]:
...
def refiner_node(state: AgentState):
    ...
    if len(current_messages) > MAX_TURNS_BEFORE_SUMMARY:
        compressed = asyncio.run(compress_history(current_messages))
