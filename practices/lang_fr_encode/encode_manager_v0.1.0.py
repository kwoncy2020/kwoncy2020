...
class EncodeManager:
    ...
    def _set_targets_name(self):
        ...
        self.v_dict["output_srt_file"] = f"{self.v_dict['suffix_s_e']}_orig_audio.srt"
        self.v_dict["output_video_srt_merged_file"] = f"{self.v_dict['output_video_file'].replace('.mkv','_with_srt.mkv')}"
        self.v_dict["output_video_srt_meta_merged_file"] = f"{self.v_dict['suffix_s_e']}.mkv"
    ...
    def _set_base_commands(self):
        ...
        # output_audio_0_name:str = self.v_dict["output_encoded_audio_file"]
        # output_audio_0_name = output_audio_0_name.replace(".opus","_0.opus")
        # self.v_dict["command_make_encoded_audio_0"] = [
        #         "ffmpeg", "-y", "-i", self.v_dict["input_video_file"], "-vn",
        #         "-c:a:0", "libopus", "-b:a", f"{self.v_dict["audio_bits"]}k", 
        #         output_audio_0_name
        #     ]
        
        # output_audio_1_name:str = self.v_dict["output_encoded_audio_file"]
        # output_audio_1_name = output_audio_1_name.replace(".opus","_1.opus")
        # self.v_dict["command_make_encoded_audio_0"] = [
        #         "ffmpeg", "-y", "-i", self.v_dict["input_video_file"], "-vn",
        #         "-c:a:1", "libopus", "-b:a", f"{self.v_dict["audio_bits"]}k", 
        #         output_audio_1_name
        #     ]
...
class CommandManager:
    def __init__(self,v_dict={}):
        ...
        self.c_dict ={
            "program" : "ffmpeg",
            "pre_exists" : "-y",
            "debug_log_file" : self.v_dict["debug_log_file"],
            "input_video_file" : self.v_dict["input_video_file"],
            "vf_filter_denoise" : self.v_dict["vf_filter_denoise"],
            "vf_filter" : self.v_dict["vf_filters"],
            "library_type" : self.v_dict["library_type"],
            "inter_video_format" : self.v_dict["inter_video_format"],
            "inter_yuv_video_file" : self.v_dict["inter_yuv_video_file"],
            "output_ivf_video_file" : self.v_dict["output_ivf_video_file"],
            "output_vvenc_266_file" : self.v_dict["output_vvenc_266_file"],
            "output_video_file" : self.v_dict["output_video_file"],
            "output_orig_audio_file" : self.v_dict["output_orig_audio_file"],
            "output_encoded_audio_file" : self.v_dict["output_encoded_audio_file"],
            "output_srt_file" : self.v_dict["output_srt_file"],
            "output_video_srt_merged_file" : self.v_dict["output_video_srt_merged_file"],
            "output_video_srt_meta_merged_file" : self.v_dict["output_video_srt_meta_merged_file"],
            "audio_library" : self.v_dict["audio_library"],
            "audio_bits" : self.v_dict["audio_bits"],
        }

    def update_c_dict(self,input_c_dict={}):
        if input_c_dict:
            for k,v in input_c_dict.items():
                self.c_dict[k] = v

    def get_num_audio_from_input_video(self):
        cmd = [
                'ffprobe',
                '-v', 'error',
                '-show_streams',
    ...
    def get_command_dict_for_audio_orig_extract(self,input_c_dict={}):
        if input_c_dict:
            self.update_c_dict(input_c_dict)
        
        self.c_dict["command"] = [
            self.c_dict["program"], 
            self.c_dict["pre_exists"], 
            "-i", 
            self.c_dict["input_video_file"], 
            "-vn",
            "-acodec",
            "copy",
            self.c_dict["output_orig_audio_file"]
        ]
        self.c_dict["comment"] = f"Save extracted original audio.  - {self.c_dict["command"]} \n"
        self.c_dict["command_cli"] = " ".join(self.c_dict["command"])
        return self.c_dict
    

    def get_command_dict_for_audio_orig_extract_all(self,input_c_dict={},num_audio=0):
        if num_audio == 0:
                # ffprobe 명령어로 스트림 정보 가져오기
            num_audio = self.get_num_audio_from_input_video()
        ...
        for i in range(num_audio):
            output_orig_audio_file:str = self.c_dict["output_orig_audio_file"]
        ...
        self.c_dict["command_sets"] = command_sets
        self.c_dict["comment_sets"] = comment_sets
        self.c_dict["command_sets_cli"] = [" ".join(i) for i in command_sets]
        return self.c_dict
    
    def get_command_dict_for_encoding_audio(self,input_c_dict={}):
        if input_c_dict:
            self.update_c_dict(input_c_dict)
        self.c_dict["command"]=[
                self.c_dict["program"], 
                ...
                # "-af", A_FILTERS,  # It's quite hard to listen even original audio. Uncertain this is cuase by audio filter or just opus. Disable filter.
                "-c:a", 
                self.c_dict["audio_library"], 
                "-b:a", 
                f"{self.v_dict["audio_bits"]}k", 
                # "-application", "voip", 
                self.v_dict["output_encoded_audio_file"]
            ]
        self.c_dict["comment"] = f"encoding audio - {self.c_dict["command"]} \n"
        self.c_dict["command_cli"] = " ".join(self.c_dict["command"])
        return self.c_dict
    
    def get_command_dict_for_encoding_audio_all(self,input_c_dict={},num_audio=0):
        if input_c_dict:
            self.update_c_dict(input_c_dict)
        ...
        comment_sets=[]
        for i in range(num_audio):
            output_encoded_audio_file:str = self.c_dict["output_encoded_audio_file"]
    ...
    def get_command_dict_for_yuv_video(self,input_c_dict={}):
        if input_c_dict:
            self.update_c_dict(input_c_dict)
        self.c_dict["command"] = [
                self.c_dict["program"], 
                ...
                "-i", 
                self.c_dict["input_video_file"], 
                "-vn" if self.c_dict["vf_filter_denoise"] else "",
                self.c_dict["vf_filter"] if self.c_dict["vf_filter_denoise"] else "",
                "-pix_fmf", self.c_dict["inter_video_format"],
                "-f", "rawvieo",
                self.c_dict["inter_yuv_video_file"]
            ]
        self.c_dict["comment"] = f"Making intermediate video - {self.c_dict["command"]} \n"
        self.c_dict["command_cli"] = " ".join(self.c_dict["command"])
        return self.c_dict
    
    def get_command_dict_for_mkv_with_inter_video_audio1(self,input_c_dict={}):
        if input_c_dict:
            self.update_c_dict(input_c_dict)
        self.c_dict["command"]=[
            self.c_dict["program"], 
            self.c_dict["pre_exists"], 
            "-i", 
            self.c_dict["output_ivf_video_file"] if self.c_dict["library_type"] in [MethodType.svtapp, MethodType.libsvtav1] else self.c_dict["output_vvenc_266_file"], 
            "-i",
            self.c_dict["output_encoded_audio_file"], 
            "-c:v", "copy",
            "-c:a", "copy",
            self.c_dict["output_video_file"]
        ]
        self.c_dict["comment"]=f"Make MKV video with single audio - {self.c_dict["command"]}"
        self.c_dict["command_cli"] = " ".join(self.c_dict["command"])
        return self.c_dict
    
    def get_command_dict_for_mkv_with_srt(self,input_c_dict={}):
        if input_c_dict:
            self.update_c_dict(input_c_dict)
        self.c_dict["command"]=[
            self.c_dict["program"], 
            ...
            "-c:s", "srt",
            "-c:t", "copy",
            "-metadata", f"comment={re.sub("\.[a-zA-Z0-9]+$","",self.c_dict["output_video_file"])}",  
            "-metadata:s:a:0", "language=fre", "-metadata:s:a:0", 'title="French Audio"', 
            "-metadata:s:s:0", "language=fre", "-metadata:s:s:0", 'title="French Subtitles"',
            self.c_dict["output_video_srt_meta_merged_file"]
        ]
        # ffmpeg -i input.mp4 -vf subtitles=subtitles.srt output.mp4
        self.c_dict["comment"]=f"Make MKV video with srt - {self.c_dict["command"]}"
        self.c_dict["command_cli"] = " ".join(self.c_dict["command"])
        return self.c_dict


    def get_command_dict_for_mkv_with_inter_video_audio_all(self,input_c_dict={},num_audio=0):
        if input_c_dict:
            self.update_c_dict(input_c_dict)
        if num_audio == 0:
            num_audio = self.get_num_audio_from_input_video()
        ...
        self.c_dict["command"] += [self.c_dict["output_video_file"]]
        #     "-i",
        #     self.c_dict["output_encoded_audio_file"], 
        #     "-c:a", "copy",
        #     self.v_dict["output_video_file"]
        # ]
        self.c_dict["comment"]=f"Make MKV video with all of audio - {self.c_dict["command"]}"
        self.c_dict["command_cli"] = " ".join(self.c_dict["command"])
        return self.c_dict
    
    
    def get_command_dict_for_incoding_video(self,input_c_dict={}):
        pass

    # def get_command_dict_for_
def main():
    ...
    em = EncodeManager(
        {
            ...
            "epnum" : "44",
            "extension" : "mkv",
            "input_video_file" : "s01_e44_svtapp_2-p_p5_10bit_s640_360_b125k_o100u10_vf4366_scd1lad60t0qp26_63aq2_a64k.mkv",
        }
    )
    cm = CommandManager(em.v_dict)
    command_dict = cm.get_command_dict_for_mkv_with_srt({
        "output_video_file" : "s01_e44_svtapp_2-p_p5_10bit_s640_360_b125k_o100u10_vf4366_scd1lad60t0qp26_63aq2_a64k.mkv",
        # "output_srt_file" : "s01_e01_s.srt",
    })
    print(command_dict)
