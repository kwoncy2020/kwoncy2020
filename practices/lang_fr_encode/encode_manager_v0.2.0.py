...
class EncodeManager:
    ...
    def get_command_dict_for_mkv_with_srt(self,input_dict={}):
        ...
        command_dict["command"]=[
            ...
            "-c:s", "srt",
            "-c:t", "copy",
            "-metadata", f"comment={re.sub("\.[a-zA-Z0-9]+$","",output_video_file)}",  
    ...
    def run_svtav1app(self, input_dict={}):
        ...
        times.append(f"{command_dict["comment"] if "comment" in command_dict else command_dict["comment_sets"][-1]} done. "+f" - {time.time()-temp_time:.2f} \n")
