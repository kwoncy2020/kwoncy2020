...
import subprocess, os, re, time, json, copy
...
class EncodeManager:
    ...
    def get_command_dict_for_audio_orig_extract(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        output_orig_audio_file = getattr(command_dict,"output_orig_audio_file",self.v_info.output_orig_audio_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_for_audio_orig_extract_all(self,input_dict={},num_audio=0):
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        output_orig_audio_file = getattr(command_dict,"output_orig_audio_file",self.v_info.output_orig_audio_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_for_encoding_audio(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        audio_encode_library = getattr(command_dict,"audio_encode_library",self.v_info.audio_encode_library)
        audio_bits = getattr(command_dict,"audio_bits",self.v_info.audio_bits)
    ...
    def get_command_dict_for_encoding_audio_all(self,input_dict={},num_audio=0):
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        audio_encode_library = getattr(command_dict,"audio_encode_library",self.v_info.audio_encode_library)
        audio_bits = getattr(command_dict,"audio_bits",self.v_info.audio_bits)
    ...
    def get_command_dict_for_yuv_video(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        vf_filter_denoise_apply = getattr(command_dict,"vf_filter_denoise_apply",self.v_info.vf_filter_denoise_apply)
        vf_filters = getattr(command_dict,"vf_filters",self.v_info.vf_filters)
        inter_video_encode_format = getattr(command_dict,"inter_video_encode_format",self.v_info.inter_video_encode_format)
        inter_yuv_video_file = getattr(command_dict,"inter_yuv_video_file",self.v_info.inter_yuv_video_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_for_mkv_with_inter_video_audio1(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        output_ivf_video_file = getattr(command_dict,"output_ivf_video_file",self.v_info.output_ivf_video_file)
        encode_library_type = getattr(command_dict,"encode_library_type",self.v_info.encode_library_type)
        output_vvenc_266_file = getattr(command_dict,"output_vvenc_266_file",self.v_info.output_vvenc_266_file)
        output_encoded_audio_file = getattr(command_dict,"output_encoded_audio_file",self.v_info.output_encoded_audio_file)
        output_video_file = getattr(command_dict,"output_video_file",self.v_info.output_video_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_for_mkv_with_srt(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        output_video_file = getattr(command_dict,"output_video_file",self.v_info.output_video_file)
        output_srt_file = getattr(command_dict,"output_srt_file",self.v_info.output_srt_file)
        output_video_srt_meta_merged_file = getattr(command_dict,"output_video_srt_meta_merged_file",self.v_info.output_video_srt_meta_merged_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_for_mkv_with_inter_video_audio_all(self,input_dict={},num_audio=0):
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        output_ivf_video_file = getattr(command_dict,"output_ivf_video_file",self.v_info.output_ivf_video_file)
        encode_library_type = getattr(command_dict,"encode_library_type",self.v_info.encode_library_type)
    ...
    def get_command_dict_add_meta_info_with_video_name(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        output_video_file = getattr(command_dict,"output_video_file",self.v_info.output_video_file)
        output_video_with_meta_file = getattr(command_dict,"output_video_with_meta_file",self.v_info.output_video_srt_meta_merged_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_encode_video_svtapp(self,input_dict={}, num_passes=2):
        # 3-pass not working correctly need more test.
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        inter_yuv_video_file = getattr(command_dict,"inter_yuv_video_file",self.v_info.inter_yuv_video_file)
        video_encode_format_depth = getattr(command_dict,"video_encode_format_depth",self.v_info.video_encode_format_depth)
        inter_stats_file = getattr(command_dict,"inter_stats_file",self.v_info.inter_stats_file)
        output_ivf_video_file = getattr(command_dict,"output_ivf_video_file",self.v_info.output_ivf_video_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_encode_video_libsvtav1(self, input_dict={}):
    ...
    def get_command_dict_encode_video_vvenc(self,input_dict={}, num_passes=2):
        command_dict = copy.deepcopy(input_dict)
        input_video_file = getattr(command_dict,"input_video_file",self.v_info.input_video_file)
        inter_yuv_video_file = getattr(command_dict,"inter_yuv_video_file",self.v_info.inter_yuv_video_file)
        inter_stats_vvenc_file = getattr(command_dict,"inter_stats_vvenc_file",self.v_info.inter_stats_vvenc_file)
        vvenc_preset = getattr(command_dict,"vvenc_preset",self.v_info.vvenc_preset)
        video_encode_format_depth = getattr(command_dict,"video_encode_format_depth",self.v_info.video_encode_format_depth)
        output_vvenc_266_file = getattr(command_dict,"output_vvenc_266_file",self.v_info.output_vvenc_266_file)
        debug_log_file = getattr(command_dict,"debug_log_file",self.v_info.debug_log_file)
    ...
    def get_command_dict_subtitle_whisper(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        whisper_model = getattr(command_dict,"whisper_model",self.v_info.whisper_model)
        # output_encoded_audio_file = getattr(command_dict,"output_encoded_audio_file",self.v_info.output_encoded_audio_file)
        output_orig_audio_file = getattr(command_dict,"output_orig_audio_file",self.v_info.output_orig_audio_file)
        debug_log_whisper_file = getattr(command_dict,"debug_log_whisper_file",self.v_info.debug_log_whisper_file)
    ...
    def get_command_dict_subtitle_whisper_x(self,input_dict={}):
        command_dict = copy.deepcopy(input_dict)
        whisper_model = getattr(command_dict,"whisper_model",self.v_info.whisper_model)
        # output_encoded_audio_file = getattr(command_dict,"output_encoded_audio_file",self.v_info.output_encoded_audio_file)
        output_orig_audio_file = getattr(command_dict,"output_orig_audio_file",self.v_info.output_orig_audio_file)
        debug_log_whisper_x_file = getattr(command_dict,"debug_log_whisper_x_file",self.v_info.debug_log_whisperx_file)
...
def main():
    ...
    def run_svt(info_dict={}):
        ...
        sleep_time:int = getattr(info_dict,"sleep_time",600)
    ...
    def run_whisper_x(info_dict={}):
        start:int = info_dict["start"]
        end:int = info_dict["end"]
        sleep_time:int = getattr(info_dict,"sleep_time",300)
